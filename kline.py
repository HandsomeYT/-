...
startdate = datetime.date(2018,6,26)
enddate = datetime.date(2019,6,26)
data=pd.DataFrame(pd.read_excel('data.xlsx',index_col='日期'))#读取数据、设置日期为index
...
stdata['DateTime'] = pd.to_datetime(stdata['DateTime'])
stdata['DateTime'] = mdates.date2num(stdata['DateTime'])#把日期转化成天数，从公元0年开始算
...
def main():
    ...
    av = pd.DataFrame(daysreshape.Close.values)
    #print(av)
    AV = av.mean()
    print(AV)
    Av1 = av.rolling(MA1).mean()
    Av2 = av.rolling(MA2).mean()
    #print(Av1,Av2)
